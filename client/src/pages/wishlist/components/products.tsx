    ...
    return (
        <div className="py-10 font-sans border-b border-gray-200 dark:bg-black dark:text-white">
            <div className="container">
                <h3 className="text-5xl text-center font-bold tracking-tight">{t("WishList")}</h3>
                {!isFavoritesLoaded ? (
                    <p className="text-center text-3xl  text-primary font-bold mt-8">{t("Your wishlist is empty.")}</p>
                ) : (
                            ...
                            {favorites?.map((favorite) => (
                                <tr className="border-b-gray-200 border-b-[1px]" key={favorite._id}>
                                    <td className="p-5 flex items-center gap-4">
                                        ...
                                        />
                                        <span className="text-gray-300 xs:text-xs sm:text-base cursor-pointer transition-all duration-300 hover:text-red-800 capitalize">
                                            {t(`products.${favorite.productId.name.replace(/\s+/g, "_").toLowerCase()}`)}
                                        </span>
                                    </td>
